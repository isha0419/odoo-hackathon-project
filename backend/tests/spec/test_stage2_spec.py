...
def create_admin_token(client: TestClient, db: Session) -> tuple[str, str]:
    ...
    response = client.post("/api/auth/signup", json={
        "name": "Admin User",
        "email": "admin_spec@assetflow.io",
        "password": "password123"
    })
    assert response.status_code == 201
    
    # Force role to ADMIN in DB
    db.execute(text(f"UPDATE users SET role = 'ADMIN' WHERE email = 'admin_spec@assetflow.io'"))
    db.commit()
...
def test_stage2_crown_jewel_double_allocation(client: TestClient, db_session: Session):
    ...
    # 1. Create Category
    cat_resp = client.post("/api/categories", headers=admin_headers, json={
        "name": "Laptops",
        "custom_fields": {}
    })
    ...
    alloc1_resp = client.post("/api/allocations", headers=admin_headers, json={
        "asset_id": asset_id,
    ...
    alloc2_resp = client.post("/api/allocations", headers=admin_headers, json={
        "asset_id": asset_id,
        "holder_user_id": emp2_id
    })
...
def test_stage2_transfer_flow(client: TestClient, db_session: Session):
    ...
    cat_id = cat_resp.json()["id"]
    asset_id = client.post("/api/assets", headers=admin_headers, json={"name": "iPhone", "category_id": cat_id}).json()["id"]
    ...
    transfer_resp = client.post("/api/transfers", headers=emp1_headers, json={
        "asset_id": asset_id,
        "to_user_id": emp2_id,
        "reason": "Team switch"
    })
