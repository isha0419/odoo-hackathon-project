...
def test_stage4_audit_cycle_flow(client: TestClient, db_session: Session):
    ...
    cat_id = client.post(
        "/api/categories", headers=admin_headers, json={"name": "Furniture", "custom_fields": {}}
    ).json()["id"]
    client.post(
        "/api/assets", headers=admin_headers, json={"name": "Office Chair", "category_id": cat_id, "location": "HQ"}
    )
    ...
    audit_resp = client.post(
        ...
        json={"name": "Q1 Furniture Audit", "start_date": "2026-01-01", "end_date": "2026-01-15"},
    )
    ...
    client.get(f"/api/audit-cycles/{audit_id}", headers=admin_headers).json()
