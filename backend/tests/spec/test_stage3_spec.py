...
def test_stage3_crown_jewel_booking_overlap(client: TestClient, db_session: Session):
    ...
    asset_resp = client.post(
        ...
        json={"name": "Conference Room B2", "category_id": cat_id, "is_bookable": True},
    )
    ...
    booking1_resp = client.post(
        "/api/bookings",
        headers=admin_headers,
        json={"asset_id": asset_id, "start": start_time.isoformat(), "end": end_time.isoformat()},
    )
    ...
    overlap_resp = client.post(
        "/api/bookings",
        headers=admin_headers,
        json={"asset_id": asset_id, "start": overlap_start.isoformat(), "end": overlap_end.isoformat()},
    )
    ...
    good_resp = client.post(
        "/api/bookings",
        headers=admin_headers,
        json={"asset_id": asset_id, "start": good_start.isoformat(), "end": good_end.isoformat()},
    )
...
def test_stage3_maintenance_kanban(client: TestClient, db_session: Session):
    ...
    """
    admin_token, _ = create_admin_token(client, db_session)
    admin_headers = {"Authorization": f"Bearer {admin_token}"}

    cat_id = client.post("/api/categories", headers=admin_headers, json={"name": "HVAC", "custom_fields": {}}).json()[
        "id"
    ]
    asset_id = client.post(
        "/api/assets", headers=admin_headers, json={"name": "AC Unit", "category_id": cat_id}
    ).json()["id"]
    ...
    maint_resp = client.post(
        "/api/maintenance",
        headers=admin_headers,
        json={"asset_id": asset_id, "issue_description": "Not cooling", "priority": "HIGH"},
    )
    ...
    appr_resp = client.post(
        f"/api/maintenance/{maint_id}/transition", headers=admin_headers, json={"to_status": "APPROVED"}
    )
    ...
    tech_resp = client.post(
        ...
        headers=admin_headers,
        json={"to_status": "TECHNICIAN_ASSIGNED", "technician_name": "Bob The Builder"},
    )
    assert tech_resp.status_code == 200

    # IN_PROGRESS
    prog_resp = client.post(
        f"/api/maintenance/{maint_id}/transition", headers=admin_headers, json={"to_status": "IN_PROGRESS"}
    )
    assert prog_resp.status_code == 200

    # RESOLVED
    res_resp = client.post(
        f"/api/maintenance/{maint_id}/transition", headers=admin_headers, json={"to_status": "RESOLVED"}
    )
