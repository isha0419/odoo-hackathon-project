...
class AssetAlreadyAllocatedException(Exception):
...
def allocate(db: Session, data: AllocateRequest, actor_id: uuid.UUID) -> Allocation:
    ...
    if active_alloc:
        ...
        raise AssetAlreadyAllocatedException(conflict_body=conflict_body)
...
def return_asset(db: Session, allocation_id: uuid.UUID, notes: Optional[str], actor_id: uuid.UUID) -> Allocation:
