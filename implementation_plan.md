...
- **Backend (Stages 0–4, Seed, Integration)** → Split between **Omm (A)** and **Satya (B)**
- **Frontend (Stage 5.2)** → Split between **Yash (C)** and **Isha (D)**
- Omm handles Stage 0 (foundation), Stage 1 (auth+org), Stage 4 (audit/reports/dashboard/notif), Stage 5.1 (seed), and Stage 6 (integration). Satya handles Stage 2 (assets/allocation/transfer — CJ#1) and Stage 3 (booking/maintenance — CJ#2).
- All stages are written as if a **single developer** is executing them. The parallelism comes from Stages 1–4 being independent tracks branching from Stage 0, and frontend running in parallel with backend. See **Parallel Build Order** at end of document.
...
## Stage 3 — Booking + Maintenance (Track B-2) — Crown Jewel #2

> **Owner**: **Satya (B)** — starts after Stage 2 PR is submitted
> **Branch**: `feature/track-b2-booking-maint`
> **Dependency**: Stage 0 complete on `main`
...
## Stage 4 — Audit + Reports + Notifications + Dashboard + Logs (Track A-2)

> **Owner**: **Omm (A)** — starts after Stage 1 PR is submitted
> **Branch**: `feature/track-a2-audit-reports`
> **Dependency**: Stage 0 complete on `main`
...
## Stage 5 — Seed Data & Frontend

> **Seed (Task 5.1)**: **Omm (A)** — after backend stages merged
> **Frontend (Tasks 5.2–5.3)**: **Yash (C)** + **Isha (D)** — start in parallel with backend from Hour 0:45
> **Branch (Seed)**: `feature/seed-data` | **Branch (Frontend)**: `feature/frontend`
> **Dependency (Seed)**: Stages 1–4 merged into `main` | **Dependency (Frontend)**: Stage 0 merged (API contracts known)
> **Owns**: Omm → `backend/app/seed/seed.py` | Yash+Isha → `frontend/*`
...
### Task 5.2 — Frontend: Screens 1–5 (Yash / C)

**Owner**: **Yash (C)**
**Files**: `frontend/pages/login.js`, `frontend/pages/dashboard.js`, `frontend/pages/org.js`, `frontend/pages/assets.js`, `frontend/pages/allocation.js`, `frontend/components/sidebar.js`, `frontend/app.js`, `frontend/styles/`, `frontend/index.html`, `frontend/utils/api.js`

> [!NOTE]
> Yash owns the **app shell** (sidebar, routing, JWT storage, API helper) and Screens 1–5. Isha builds Screens 6–10 as page modules that plug into the same shell. Coordinate on `frontend/app.js` routing table.

**Shared setup (Yash creates first):**
- `index.html` with dark theme root, font imports (Inter/Outfit from Google Fonts)
- `styles/` — CSS design system: dark background (#0f1117), card glassmorphism, status badge colors (green=Active, red=Inactive/Lost, orange=Maintenance, blue=Allocated), transitions
- `utils/api.js` — fetch wrapper that auto-injects `Authorization: Bearer {token}` from localStorage, handles 401→redirect to login
- `app.js` — SPA router, sidebar toggle, imports all page modules
- `components/sidebar.js` — Left nav matching mockup (Dashboard, Org Setup, Assets, Allocation & Transfer, Resource Booking, Maintenance, Audit, Reports, Notifications)

**Screen 1 — Login/Signup**: Dark-themed card with AF logo, email/password fields, "Forgot password" link, signup note ("creates employee account, admin roles assigned later"), "Create Account" button. Stores JWT in localStorage on success.

**Screen 2 — Dashboard**: "Today's Overview" KPI cards (Available, Allocated, Available counts + Active Bookings, Pending Transfers, Upcoming Returns), overdue returns alert banner (red), quick-action buttons (Register Asset, Book Resource, Raise Request), Recent Activity feed.

**Screen 3 — Organization Setup (Admin)**: Tab navigation (Departments, Categories, Employees, + Add). Departments table (name, head, parent dept, status badges). Categories table with custom_fields. Employee list with role-promotion action.

**Screen 4 — Asset Registry**: Search bar (tag, serial, QR code) + "Register Asset" button. Filter pills (Category, Status, Department). Assets table (Tag, Name, Category, Status badge, Location).

**Screen 5 — Allocation & Transfer**: Asset detail view showing allocation status. Red conflict banner ("Already allocated to Priya Shah (Engineering)") on 409. "Transfer Request" flow with from/to employee selectors. Allocation history table below.

**Tech stack**: Vanilla HTML/CSS/JS or Vite + vanilla. Dark theme matching mockup. Responsive.

**Acceptance Criteria:**
- [ ] App shell: sidebar nav, SPA routing, JWT auth flow all working
- [ ] Dark theme with status badges matching mockup
- [ ] Screen 1: login/signup functional, JWT stored
- [ ] Screen 2: dashboard KPI cards populated from `GET /dashboard`
- [ ] Screen 3: org setup CRUD functional (admin-only)
- [ ] Screen 4: asset search/filter/register working
- [ ] Screen 5: allocation with 409 conflict banner, transfer flow

---

### Task 5.3 — Frontend: Screens 6–10 (Isha / D)

**Owner**: **Isha (D)**
**Files**: `frontend/pages/booking.js`, `frontend/pages/maintenance.js`, `frontend/pages/audit.js`, `frontend/pages/reports.js`, `frontend/pages/notifications.js`

> [!NOTE]
> Isha builds page modules that export an `init()` or `render()` function. Yash's `app.js` imports and routes to them. Use the shared `utils/api.js` for all API calls. Do NOT modify sidebar or app shell — just add your page files.

**Screen 6 — Resource Booking**: Asset selector/header (Conference Room B2). Time slot grid showing existing bookings (green = booked, red = conflict). Overlap rejection message on 409. "Book a slot" form with start/end time pickers.

**Screen 7 — Maintenance Kanban**: Column board — Pending | Approved | Technician Assigned | In Progress | Resolved. Cards show asset tag, issue summary, priority badge. "Raise Request" button. Transition buttons on cards (Approve, Assign Tech, etc.).
...
**Screen 9 — Reports**: Charts — Utilization by Department (bar chart), Maintenance Frequency (bar chart). Use Canvas/SVG or lightweight chart lib. Text sections: Most Used Assets, Idle Assets, Assets Due for Maintenance/Retirement. "Export CSV" button wired to `GET /reports/export?report=...`.

**Screen 10 — Notifications & Activity Logs**: Tab filters (All, Alerts, Approvals, Bookings). Notification list with icons, timestamps, read/unread states. Mark-as-read button. Activity log table with actor, action, entity, timestamp.

**Acceptance Criteria:**
- [ ] Screen 6: booking grid with overlap 409 shown, slot creation works
- [ ] Screen 7: kanban board renders, cards move on transition
- [ ] Screen 8: audit checklist with verification badges, close cycle
- [ ] Screen 9: charts render, CSV export downloads
- [ ] Screen 10: notification list with read/unread, activity log
- [ ] All pages use shared `api.js` and match dark theme
...
- [ ] Yash's shell + Screens 1–5 integrated with Isha's Screens 6–10
...
> [!IMPORTANT]
> **Backend** is split between Omm (A) and Satya (B). **Frontend** is split between Yash (C) and Isha (D). Frontend starts in parallel with backend right after Stage 0.

| Stage | Track | Owner | Branch | Duration Est. |
|---|---|---|---|---|
| **Stage 0** | Skeleton & Foundation | **Omm (A)** | `main` | ~45 min |
| **Stage 1** | Auth + Org + Employees | **Omm (A)** | `feature/track-a-auth-org` | ~1.5 hr |
| **Stage 2** | Assets + Allocation + Transfer (CJ#1) | **Satya (B)** | `feature/track-b-assets-alloc` | ~2 hr |
| **Stage 3** | Booking + Maintenance (CJ#2) | **Satya (B)** | `feature/track-b2-booking-maint` | ~2 hr |
| **Stage 4** | Audit + Reports + Notif + Dashboard | **Omm (A)** | `feature/track-a2-audit-reports` | ~2.5 hr |
| **Stage 5.1** | Seed Data | **Omm (A)** | `feature/seed-data` | ~1 hr |
| **Stage 5.2** | Frontend Screens 1–5 + App Shell | **Yash (C)** | `feature/frontend` | ~3 hr |
| **Stage 5.3** | Frontend Screens 6–10 | **Isha (D)** | `feature/frontend` | ~3 hr |
| **Stage 6** | Integration & Demo | **Omm (A)** | `main` | ~1 hr |

### Parallelization Timeline

```
Hour 0    ─── Stage 0 (Omm pushes to main) ─────────────────┐
                                                              │
Hour 0:45 ─── Branch point ─────────────────────────────────┤
              │                                               │
     BACKEND  │                              FRONTEND         │
              ├── Stage 1 (Omm) ──────┐    ┌── Task 5.2      │
              ├── Stage 2 (Satya) ────┤    │   (Yash:        │
              │                       │    │    shell +       │
              │   Omm finishes S1 →   │    │    Screens 1-5) │
              │   starts Stage 4      │    │                  │
              │                       │    ├── Task 5.3      │
              │   Satya finishes S2 → │    │   (Isha:        │
              │   starts Stage 3      │    │    Screens 6-10)│
              │                       │    │                  │
              ├── Stage 3 (Satya) ────┤    │                  │
              ├── Stage 4 (Omm) ──────┤    │                  │
              │                       │    │                  │
Hour 4:00 ─── Backend PRs merged ────┤    └─── FE PR ───────┤
              │                                               │
Hour 4:00 ─── Stage 5.1 Seed (Omm) ─────────────────────────┤
              │                                               │
Hour 5:00 ─── Stage 6 Integration + Demo (Omm) ─────────────┤
              │                                               │
Hour 6:00 ─── SHIP ─────────────────────────────────────────┘
```
...
| Owner | Files Owned (exclusive write) |
|---|---|
| **Omm (A)** — Stage 0 | `main.py`, `config.py`, `db.py`, `security.py`, `deps.py`, `alembic/*`, `models/*`, `docker-compose.yml`, `.env.example`, `Dockerfile`, `requirements.txt` |
| **Omm (A)** — Stage 1 | `routers/auth.py`, `routers/org.py`, `services/auth_service.py`, `services/org_service.py`, `schemas/auth.py`, `schemas/org.py` |
| **Satya (B)** — Stage 2 | `routers/assets.py`, `routers/allocation.py`, `routers/transfers.py`, `services/asset_service.py`, `services/allocation_service.py`, `services/transfer_service.py`, `schemas/assets.py`, `schemas/allocation.py`, `schemas/transfer.py` |
| **Satya (B)** — Stage 3 | `routers/booking.py`, `routers/maintenance.py`, `services/booking_service.py`, `services/maintenance_service.py`, `schemas/booking.py`, `schemas/maintenance.py` |
| **Omm (A)** — Stage 4 | `routers/audit.py`, `routers/reports.py`, `routers/notifications.py`, `routers/dashboard.py`, `routers/activity_logs.py`, `services/audit_service.py`, `services/report_service.py`, `services/dashboard_service.py`, `services/notifications_service.py` (body), `services/activity_service.py` (body), `schemas/audit.py`, `schemas/reports.py`, `schemas/notifications.py`, `schemas/dashboard.py` |
| **Omm (A)** — Stage 5.1 | `seed/seed.py` |
| **Yash (C)** — Task 5.2 | `frontend/index.html`, `frontend/app.js`, `frontend/styles/*`, `frontend/utils/*`, `frontend/components/*`, `frontend/pages/login.js`, `frontend/pages/dashboard.js`, `frontend/pages/org.js`, `frontend/pages/assets.js`, `frontend/pages/allocation.js` |
| **Isha (D)** — Task 5.3 | `frontend/pages/booking.js`, `frontend/pages/maintenance.js`, `frontend/pages/audit.js`, `frontend/pages/reports.js`, `frontend/pages/notifications.js` |

---

## Appendix A — Cross-Track Dependencies (Frozen Signatures)

These two functions are defined in Stage 0 with stub bodies. Omm fills the bodies in Stage 4. All other backend stages import and call them.
...
## Appendix C — Parallel Build Order (Start Coding Instructions)

> [!IMPORTANT]
> **All 4 teammates can start coding simultaneously after Stage 0 lands on `main`.** This section tells each person exactly what to do and in what order.

### Omm (A) — Backend Lead & Integration

| Order | What | Branch | Notes |
|---|---|---|---|
| 1 | **Stage 0** — Push skeleton to `main` | `main` | Everyone waits on this (~45 min) |
| 2 | **Stage 1** — Auth + Org | `feature/track-a-auth-org` | PR → `main` when done |
| 3 | **Stage 4** — Audit, Reports, Dashboard, Notif, Logs | `feature/track-a2-audit-reports` | Start immediately after Stage 1 PR. Fills activity/notification stubs. |
| 4 | **Stage 5.1** — Seed script | `feature/seed-data` | After all backend merged |
| 5 | **Stage 6** — Merge all, run demo | `main` | Final integration |

> **If anything major breaks** (merge conflicts, schema issues, cross-track bugs) — it falls on Omm to resolve.

### Satya (B) — Backend: Assets & Booking

| Order | What | Branch | Notes |
|---|---|---|---|
| 1 | **Stage 2** — Assets + Allocation + Transfer (Crown Jewel #1) | `feature/track-b-assets-alloc` | Start right after Stage 0. This is the highest-risk backend stage. |
| 2 | **Stage 3** — Booking + Maintenance (Crown Jewel #2) | `feature/track-b2-booking-maint` | Start after Stage 2 PR is submitted. |

> Both crown jewels are on Satya — prioritize getting the DB constraints + 409 responses right.

### Yash (C) — Frontend: Shell + Screens 1–5

| Order | What | Branch | Notes |
|---|---|---|---|
| 1 | **App shell** — `index.html`, CSS design system, sidebar, router, `api.js` | `feature/frontend` | Start right after Stage 0. Build against API contracts from `design.md` Section 11 — use mock/stub data until backend is ready. |
| 2 | **Screen 1** — Login/Signup | same branch | Wire to `POST /auth/login`, `POST /auth/signup` |
| 3 | **Screen 2** — Dashboard | same branch | Wire to `GET /dashboard` |
| 4 | **Screen 3** — Org Setup | same branch | Wire to departments/categories/employees CRUD |
| 5 | **Screen 4** — Asset Registry | same branch | Wire to `GET /assets`, `POST /assets` |
| 6 | **Screen 5** — Allocation & Transfer | same branch | Wire to allocation/transfer endpoints, show 409 conflict |

> Yash creates the shared shell first so Isha can import it. Coordinate on the routing table in `app.js`.

### Isha (D) — Frontend: Screens 6–10

| Order | What | Branch | Notes |
|---|---|---|---|
| 1 | **Screen 6** — Resource Booking | `feature/frontend` | Start right after Yash commits the app shell. Build page module with `render()` export. |
| 2 | **Screen 7** — Maintenance Kanban | same branch | Kanban columns with drag/transition buttons |
| 3 | **Screen 8** — Audit | same branch | Checklist table with verification badges |
| 4 | **Screen 9** — Reports | same branch | Bar charts + CSV export |
| 5 | **Screen 10** — Notifications & Activity Logs | same branch | Notification list + activity table |

> Isha's pages are independent modules. Use `utils/api.js` for all API calls. Match the dark theme from Yash's CSS.

### Coordination Rules

1. **Frontend can start before backend is done** — build screens against the API contracts in `design.md` Section 11. Use hardcoded mock responses initially, swap to real API when backend lands.
2. **Yash and Isha share `feature/frontend` branch** — Yash owns `app.js`, `styles/`, `utils/`, `components/`, `pages/login|dashboard|org|assets|allocation`. Isha owns only `pages/booking|maintenance|audit|reports|notifications`. No overlap.
3. **Satya works serially** (Stage 2 → Stage 3) because both are backend and Stage 3 can reuse patterns from Stage 2.
4. **Omm works serially** (Stage 0 → 1 → 4 → 5.1 → 6) and handles all merges and any cross-track breakage.
5. **Major issues fall on Omm (A)** — merge conflicts, schema problems, cross-track dependency bugs.
